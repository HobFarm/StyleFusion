...
const SettingsPanel: React.FC<SettingsPanelProps> = ({
  ...
  const handleTestConnection = async () => {
    ...
    onTextModelIdChange(localTextModelId);
    onImageModelIdChange(localImageModelId);
    const success = await onTestConnection(localKey);
    if (success) {
      await onFetchModels();
    }
  };
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded-md text-sm text-slate-200 focus:outline-none focus:border-blue-500 disabled:opacity-50"
            >
              {textModels.length === 0 ? (
                <option value={localTextModelId}>{localTextModelId}</option>
              ) : (
            ...
            {textModels.length > 0 && !textModels.find(m => m.id === localTextModelId) && (
          ...
          <div>
            ...
            >
              {imageModels.length === 0 ? (
                <option value={localImageModelId}>{localImageModelId}</option>
              ) : (
            ...
            {imageModels.length > 0 && !imageModels.find(m => m.id === localImageModelId) && (
              <p className="mt-1 text-xs text-amber-400">
                Selected model not found in available models.
              </p>
            )}
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              disabled={isLoadingModels || !isKeyValid}
              className="flex items-center gap-2 px-3 py-2 bg-slate-700 hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed text-slate-200 rounded-md transition-colors"
